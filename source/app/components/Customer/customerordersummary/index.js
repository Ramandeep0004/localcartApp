...
import {
    LogBox, RefreshControl, ScrollView, View
} from 'react-native';
...
const CustomerOrderSummary = props => {
    ...
    const [handleScanPaymentText, setHandleScanPaymentText] = useState('');
                ...
                <View style={styles.main}>
                    <View style={base.container}>
                                ...
                                : orderDetails && parseInt(orderDetails.request_status) === 1 ?
                                    <View style={{ flexDirection: 'row', justifyContent: 'center', alignItems: 'center' }}>
                                        ...
                                        <View style={styles.viewpending222}>
                                            <Text style={[styles.delivery, { color: orderDetails && parseInt(orderDetails.request_status) == 1 ? colors.green : colors.lightgrey }]}>Accepted</Text>
                                        </View>
                                    </View> : orderDetails && parseInt(orderDetails.request_status) === 2 ?
                                        <View style={{ flexDirection: 'row', justifyContent: 'center', alignItems: 'center' }}>
                                            <Text style={styles.edit}>{t("orderSummary.Edit request already submitted")}</Text>
                                            <View style={styles.viewpending33}>
                                                <Text style={[styles.delivery, { color: orderDetails && parseInt(orderDetails.request_status) == 2 ? colors.red : colors.lightgrey }]}>Declined</Text>
                                            </View>
                                        </View> :
                                        <>
                                            {orderDetails && parseInt(orderDetails && orderDetails.order_status) === 0 ?
                                                <TouchableOpacity onPress={() => props.navigation.navigate('customerorderedit', { item: orderDetails && orderDetails.id })}>
                        ...
                        <View >
                            <EditOrderSumList
                                item={orderDetails}
                                action={() => props.navigation.navigate('customerorderedit', { item: orderDetails && orderDetails.id })}
                            />
                        </View>
                        ...
                        {orderDetails && parseInt(orderDetails && orderDetails.order_status) === 0 ? <View style={styles.button}>
                            <Button title={t('orderSummary.Cancel order')} buttonStyle={styles.buttonstyle} titleStyle={styles.Title} onPress={() => setCancelOrder(true)} />
                        </View> : null}

                        {orderDetails && (parseInt(orderDetails && orderDetails.order_status) === 1 || parseInt(orderDetails && orderDetails.order_status) === 2) && !isEmpty(orderDetails.payment_method) ? <View style={styles.button}>
                            <Button title={t('orderSummary.Refuse order')} buttonStyle={styles.buttonstyle} titleStyle={styles.Title} onPress={() => setCancelOrder(true)} />
                        </View> : null}

                        {orderDetails && (parseInt(orderDetails.order_status) === 1 || parseInt(orderDetails.order_status) === 2 || parseInt(orderDetails.order_status) === 3) && isEmpty(orderDetails.payment_method) ? <View style={styles.mainbtn}>
                            <View style={styles.btnone}>
                                <Button title={t('orderSummary.Refuse order')} buttonStyle={styles.buttonone} titleStyle={styles.title} onPress={() => setCancelOrder(true)} />
                            </View>
                            <View style={styles.btntwo}>
                                <Button title={t('orderSummary.Pay for order')} buttonStyle={styles.buttontwo} onPress={() => setPayment(true)} />
                            </View>
                        </View> :
                            null}

                        {/* {orderDetails && parseInt(orderDetails && orderDetails.order_status) === 2 && !isEmpty(orderDetails.payment_method) ? <View style={styles.button}>
                            <Button title={t('orderSummary.Cancel order')} buttonStyle={styles.buttonstyle} titleStyle={styles.Title} onPress={() => setCancelOrder(true)} />
                        </View> : null} */}
                        {/* {orderDetails && (parseInt(orderDetails.order_status) === 2 || parseInt(orderDetails.order_status) === 3) && isEmpty(orderDetails.payment_method) ? <View style={styles.viewbtn}>
                            <Button title={t('orderSummary.Pay for order')} buttonStyle={styles.buttonpay} onPress={() => setPayment(true)} />
                        </View> : null} */}
                        {/* OrderSummary -> edit order-> delivery-> Upload bill & Repeat Order */}
                        {
                            orderDetails && parseInt(orderDetails.order_status) === 3 && isEmpty(orderDetails.printed_bill_slip) && !isEmpty(orderDetails.payment_method) ?
                                <View style={styles.mainbtn}>
                                    <View style={styles.btnone}>
                                        <Button title={t('orderSummary.Upload bill')} buttonStyle={styles.buttonone} titleStyle={styles.title} onPress={() => setUpload(true)} />
                                    </View>
                                    <View style={styles.btntwo}>
                                        <Button title={t('orderSummary.Repeat order')} buttonStyle={styles.buttontwo} onPress={() => setRepeat(true)} />
                                    </View>
                                </View>
                                : null
                        }
                    ...
                    {repeat ? <RepeatOrder
                        ...
                        navigation={props.navigation}
                        action={() => {
                            props.navigation.navigate('customerorderedit', { item: orderDetails && orderDetails.id, repeat: 'repeat' })
                            setRepeat(false)
                        }}
                    /> : null}
