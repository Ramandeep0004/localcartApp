...
import MinimumAmountModal from '../../../CustomerComponent/minimumAmount';
...
import { checkCurrentDAte, checkCurrentTimedate, comapareTwoTimes, isEmptyObj, renderImage, UrlName } from '../../Helper/general';
import ImagePickerModal from '../../Helper/imagePicker';
import Loader from '../../Helper/loader';
import { finaltotalPrice, finaltotalPricePercentage, finaltotalPriceType, totalPrice, subTotalPrice, findPercentage } from '../../Helper/orderHelpers';
import { Toaster } from '../../Helper/Toaster';
import Validation from '../../Helper/Validations';
import styles from './style';

const CustomerMyCart = props => {
  ...
  const submit = async () => {
    ...
    if (isValid && !isValid.haveError) {
      if (checkCurrentDAte(values.deliveryDate) && checkCurrentTimedate(values.deliveryTime)) {
        ...
        if (sortValue === 'Home Delivery' && values.address === null) {
          ...
          if (response && response.status) {
            ...
            setLoader(false);
            props.navigation.navigate('customerordersummary', { item: response.order.id })
            setValues({
              ...values,
              shopId: null,
              itemsArr: [],
  ...
  const onConfirm = () => {
    props.navigation.navigate('customerwallet')
  };
        ...
        <ScrollView nestedScrollEnabled={true}
          ...
          <View style={styles.main}>
                ...
                <View style={base.col12}>
                  <View style={[base.row, { justifyContent: 'center', alignItems: 'center' }]}>
                    <View style={styles.MainImage}>
                      <TouchableOpacity onPress={() => {
                        props.navigation.navigate('customerstore', { item: shopDetail })
                      }}>
                    ...
                    <View style={styles.MainView}>
                      <View style={styles.Vone}>
                        {
                          shopDetail && shopDetail.shop_name ?
                            <TouchableOpacity onPress={() => {
                              props.navigation.navigate('customerstore', { item: shopDetail })
                            }}>
                              <Text style={styles.bablu}>{shopDetail.shop_name}</Text>
                            </TouchableOpacity>
                            : null
                        }
                        {
                          shopDetail && shopDetail.shopkeeper_type && shopDetail.user ?
                            <Text style={styles.arnav}>{shopDetail.user && shopDetail.user.first_name ? shopDetail.user.first_name
                              : null
                            }{' '}{
                                shopDetail.user && shopDetail.user.last_name ? shopDetail.user.last_name
                                  : null
                              }{' '}({shopDetail.shopkeeper_type ? shopDetail.shopkeeper_type.title : null}) </Text> : null}
                        ...
                        <View>
                          {
                            shopDetail && parseInt(shopDetail.status) === 1 ?
                              <TouchableOpacity onPress={() => handleCallAcivity(shopDetail.user.user_phonenumber)}>
                              ...
                              :
                              <TouchableOpacity style={styles.Viewicon2}>
                                <Tooltip
                                  visible={open}
              ...
              <View style={base.col12}>
                <View style={styles.viewbonn}>
                  <CartBonnList
                    ...
                    action={(item) => (setProductDetailId({ id: item.id, shop_id: item.shop_id }), setProductDetailPopup(true))}
            ...
            <View style={base.container}>
              ...
              {shopDetail && parseInt(shopDetail.status) === 0 ?
                <View style={styles.shopOff}>
                  <View style={styles.nodelivery}>
                    <Text style={styles.fusce22}>{t("orderSummary.The shop is closed now, your order will be deliverd in next date.")}</Text>
                  </View>
                </View> : null}
            ...
            {order ?
              <OrderLater
                ...
                toScreen={() => props.navigation.reset({
                  index: 1,
                  routes: [{ name: 'customerhome' }, { name: 'customersavedorder' }]
                })}
          ...
          />
          {
            walletPopup ? <MinimumAmountModal
              open={walletPopup}
              close={() => setWalletPopup(false)}
              onConfirm={() => onConfirm()}
...
export default connect(mapStateToProps)(CustomerMyCart);
